...
export default function SiteChrome({ children }: { children: ReactNode }) {
  ...
  return <LazyMotion features={domAnimation}><ProblemFlowProvider><SiteHeader /><m.div key={pathname} className="page-transition" initial={pathname === "/" || reduceMotion ? false : { opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : MOTION_DURATION.page, ease: MOTION_EASE }}>{children}</m.div><SiteFooter /></ProblemFlowProvider></LazyMotion>;
}
